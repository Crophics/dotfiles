...
parser.add_argument('--json-output', type=str, default=None, help='file path to also write the material colors as snake_case JSON (for MaterialThemeLoader.qml)')
...
if args.debug == False:
    ...
    if args.json_output is not None:
        ...
        # Only the roles Appearance.qml's m3colors actually declares as
        # properties - QML errors ("Cannot assign to non-existent property")
        # if the JSON has extras like the paletteKeyColor roles.
        VALID_M3_ROLES = {
            ...
            'error', 'onError', 'errorContainer', 'onErrorContainer', 'primaryFixed',
            ...
            'onTertiaryFixed', 'onTertiaryFixedVariant', 'success', 'onSuccess',
            'successContainer', 'onSuccessContainer',
        }
        with open(args.json_output, 'w') as f:
            json.dump({to_snake_case(c): code for c, code in material_colors.items() if c in VALID_M3_ROLES}, f, indent=2)
else:
    if args.path is not None:
        print('\n--------------Image properties-----------------')
