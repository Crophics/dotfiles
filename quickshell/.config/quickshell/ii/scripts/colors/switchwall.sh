...
handle_kde_material_you_colors() {
    ...
    # Wrapper + its underlying `kde-material-you-colors` tool are both missing
    # (not recoverable from any local backup) - guard so this just no-ops
    # instead of erroring on every wallpaper change until it's rebuilt.
    [[ -f "$kde_wrapper" ]] && "$kde_wrapper" --scheme-variant "$kde_scheme_variant"
}
...
post_process() {
    ...
    handle_kde_material_you_colors &
    "$SCRIPT_DIR/qt6ct/set-qt6ct-colors.sh" &
    "$SCRIPT_DIR/code/material-code-set-color.sh" &
    "$SCRIPT_DIR/firefox/set-firefox-colors.sh" &
    "$SCRIPT_DIR/zen/set-zen-colors.sh" &
    # fastfetch regen disabled: fastfetch-theme.jsonc/logo-theme.txt are the old
    # pink-accent layout, which was intentionally replaced by a static boxed
    # config.jsonc the user actually uses now. Re-enabling this would silently
    # overwrite that on the next wallpaper change.
    # "$SCRIPT_DIR/fastfetch/set-fastfetch-colors.sh" &
}
...
switch() {
    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        # check_and_prompt_upscale "$imgpath" &  # disabled: don't notify about low-res wallpapers
        kill_existing_mpvpaper
    ...
    # colors.json is what MaterialThemeLoader.qml (the bar/shell's own theme)
    # reads - previously only skwd-wall's separate integration ever wrote
    # this, so the shell stopped re-theming once wallpaper handling reverted
    # to native ii and skwd stopped running.
