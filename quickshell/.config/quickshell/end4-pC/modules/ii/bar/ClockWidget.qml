...
BarWidgetSwitcher {
    ...
    rowDefault: Component {
        RowLayout {
            spacing: 4
            // RowLayout top-aligns children by default rather than
            // centering them - harmless when every child is the same
            // font.pixelSize, but the date/bullet here are "small" while
            // the time is "large", so without an explicit Layout.alignment
            // the shorter text sits above the taller time's vertical
            // center instead of level with it.
            StyledText {
